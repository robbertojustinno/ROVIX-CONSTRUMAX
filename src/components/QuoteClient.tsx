...
export default function QuoteClient(){
  ...
  async function convert(id:number){const r=await fetch("/api/quotes/convert",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({quote_id:id,payment_method:"CASH"})});if(!r.ok){setErr((await r.json()).error);return}load()}
  return <><h1>Orçamentos</h1><div className="card"><div className="form">
    <select className="input" value={customerId} onChange={e=>setCustomerId(e.target.value)}><option value="">Cliente balcão</option>{customers.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select>
    <select className="input" value={warehouseId} onChange={e=>setWarehouseId(e.target.value)}><option value="">Selecione o depósito</option>{warehouses.map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select>
  ...
  <div className="card"><table className="table"><thead><tr><th>ID</th><th>Cliente</th><th>Total</th><th>Status</th><th>Ação</th></tr></thead><tbody>{quotes.map(q=><tr key={q.id}><td>{q.id}</td><td>{q.customer||"Balcão"}</td><td>{q.total}</td><td>{q.status}</td><td>{q.status==="OPEN"&&<button className="btn" onClick={()=>convert(q.id)}>Converter em venda</button>}</td></tr>)}</tbody></table></div></>
}
