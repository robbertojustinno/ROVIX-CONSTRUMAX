...
$Root = $PSScriptRoot
$PgHome = Join-Path $Root "runtime\postgresql"
...
New-Item -ItemType Directory -Force -Path $ConfigDir,$LogsDir,(Join-Path $Root "data") | Out-Null
...
if(!$online){
  Write-Host "O sistema não respondeu. Verifique logs\app-error.log" -ForegroundColor Red
...
Write-Host ""
Write-Host "ROVIX CONSTRUMAX iniciado com sucesso." -ForegroundColor Green
Write-Host "Acesse: $url"
Write-Host "Para encerrar, use PARAR_CONSTRUMAX.bat"
